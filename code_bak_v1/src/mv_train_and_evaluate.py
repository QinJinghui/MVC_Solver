...
def train_mvlm2seq(input_batch, input_length,  attention_mask_batch, token_type_ids_batch, target_batch, target_length,
                   ...
                   encoder, decoder, encoder_optimizer, decoder_optimizer, output_lang, clip=0,
    ...
    loss = loss_v0 + loss_v1 + consistency_loss  # / 2
...
def evaluate_mvlm2seq(input_seq, input_length, attention_mask_batch, token_type_ids_batch, num_list, copy_nums,
    ...
    return all_outputs, all_outputs_v1


def train_mvlm2tree(input_batch, input_length, attention_mask_batch, token_type_ids_batch, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
                    encoder, predict, generate, merge, encoder_optimizer, predict_optimizer, generate_optimizer,
                    merge_optimizer, output_lang, num_pos, var_nums=[], clip=0.0,
                    grad_acc=False, zero_grad=True, grad_acc_steps=1, english=False,
    ...
    loss = loss_v0 + loss_v1 + consistency_loss
    loss.backward()
    # clip the grad
    if clip > 0:
        torch.nn.utils.clip_grad_norm_(encoder.parameters(), clip)
...
def evaluate_mvlm2tree(input_batch, input_length, attention_mask_batch, token_type_ids_batch, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,
    ...
    padding_hidden_v1 = torch.FloatTensor([0.0 for _ in range(predict.hidden_size)]).unsqueeze(0)
    ...
    if beam_search:
        ...
        return beams[0].out, beams_v1[0].out
    else:
        all_node_outputs = []
        for t in range(max_length):
            num_score, op, current_embeddings, current_context, current_nums_embeddings = predict(
        ...
        return all_node_outputs[0], all_node_outputs_v1[0]
...
#                     generate_nums, batch_graph, encoder, decoder, encoder_optimizer, decoder_optimizer, output_lang, clip=0,
...
#                      encoder, predict, generate, merge, encoder_optimizer, predict_optimizer, generate_optimizer,
#                      merge_optimizer, output_lang, num_pos, batch_graph, var_nums=[], clip=0.0,
#                      grad_acc=False, zero_grad=True, grad_acc_steps=1, english=False):
#     # sequence mask for attention
#     seq_mask = []
#     max_len = max(input_length)
...
#                        generate_optimizer, output_lang, num_pos, var_nums=[], clip=0.0, english=False):
#     # sequence mask for attention
#     seq_mask = []
#     max_len = max(input_length)
#     for i in input_length:
