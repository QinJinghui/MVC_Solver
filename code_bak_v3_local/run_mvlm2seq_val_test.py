...
if dataset_name == "Math23K":
    ckpt_dir = "Math23K_mv_b2s_val_test"
    data_dir = "./dataset/math23k/"
    data_path = data_dir + "Math_23K.json"
    prefix = '23k_processed.json'
elif dataset_name == "Math23K_char":
    ckpt_dir = "Math23K_char_mv_b2s_val_test"
    data_dir = "./dataset/math23k/"
    data_path = data_dir + "Math_23K_char.json"
    prefix = '23k_processed.json'
elif dataset_name == "cm17k":
    var_nums = ['x', 'y']
    ckpt_dir = "cm17k_mv_b2s_val_test"
...
if not use_share_decoder:
    decoder_optimizer_v1 = torch.optim.Adam(decoder.parameters(), lr=learning_rate, weight_decay=weight_decay)
else:
...
if not use_share_decoder:
    decoder_scheduler_v1 = torch.optim.lr_scheduler.StepLR(decoder_optimizer, step_size=max(n_epochs//4,1), gamma=0.5)
else:
...
for epoch in range(n_epochs):
    ...
    for idx in range(len(input_lengths)):
        ...
        for step in range(grad_acc_steps):
            ...
            loss = train_mvlm2seq(input_batches[idx][start_idx:end_idx],
                                ...
                                output_lang, use_clip=use_clip, clip=gclip, use_teacher_forcing=use_teacher_forcing, beam_size=1,
    ...
    if epoch % 1 == 0 or epoch > n_epochs - 5:
        ...
        # num_pos = []
            ...
            #         num_pos.append(idx)
            test_res, test_score, test_res_v1, test_score_v1 = evaluate_mvlm2seq(tokens_dict["input_ids"], len(tokens_dict["input_ids"]),
        ...
        if not use_share_decoder:
            torch.save(decoder_v1.state_dict(), os.path.join(save_dir, "lm2seq_attn_decoder_v1"))
        if best_val_acc < value_ac:
            ...
            if not use_share_decoder:
                torch.save(decoder_v1.state_dict(), os.path.join(save_dir, "lm2seq_attn_decoder_v1_best_val_acc"))
        ...
        #     if not use_share_decoder:
        #         torch.save(decoder_v1.state_dict(), os.path.join(save_dir, "lm2seq_attn_decoder_v1_best_equ_acc"))

        if best_val_acc_v1 < value_ac_v1:
            best_val_acc_v1 = value_ac_v1
            current_best_val_acc_v1 = (equation_ac_v1, value_ac_v1, eval_total)
            torch.save(encoder.state_dict(), os.path.join(save_dir, "lm2seq_attn_encoder_best_val_acc_v1"))
            torch.save(decoder.state_dict(), os.path.join(save_dir, "lm2seq_attn_decoder_best_val_acc_v1"))
            if not use_share_decoder:
                torch.save(decoder_v1.state_dict(), os.path.join(save_dir, "lm2seq_attn_decoder_v1_best_val_acc_v1"))

        # if best_equ_acc_v1 < equation_ac_v1:
        #     best_equ_acc_v1 = equation_ac_v1
        #     torch.save(encoder.state_dict(), os.path.join(save_dir, "lm2seq_attn_encoder_best_equ_acc_v1"))
        #     torch.save(decoder.state_dict(), os.path.join(save_dir, "lm2seq_attn_decoder_best_equ_acc_v1"))
        #     if not use_share_decoder:
        #         torch.save(decoder_v1.state_dict(), os.path.join(save_dir, "lm2seq_attn_decoder_v1_best_equ_acc_v1"))
